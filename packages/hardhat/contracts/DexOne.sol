...
contract DexOne {
    ...
    constructor(address token_add) {
        token = IERC20(token_add); //specifies the token address that will hook into the interface and be used through the variable 'token'
    }
    ...
    function quickEthPrice() external view returns(uint256 output) {
        uint256 feeAdjusted = 1 ether * 997;
        uint256 numerator = feeAdjusted * token.balanceOf(address(this));
        uint256 denominator = (address(this).balance * 1000) + feeAdjusted;
        return (numerator / denominator);
    }
